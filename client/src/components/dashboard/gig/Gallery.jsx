...
export default function Gallery({ formData, setFormData }) {
  ...
  const handleSubmit = async (e) => {
    ...
    if (!formData?.images.length > 0) {
      toast.error("upload minimum 1 image or video!");
      return;
    }
    setSubmitLoading(true);
    try {
      const { data } = await axios.post("/api/gig/create", formData);
      if (data) {
        toast.success(data);
        navigation("/dashboard?tab=all-gig", {
          state: { success: data },
        });
          ...
          <Card>
            ...
            <div className=" flex items-center gap-5">
              ...
              <Button
                ...
                color="success"
                className=" w-28">
                {submitLoading ? <p>Publishing...</p> : <p>Publish</p>}
              </Button>
            </div>
          </Card>
        </form>
