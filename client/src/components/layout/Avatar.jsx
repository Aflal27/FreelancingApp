...
export default function Avatar({ name, image, width, height, userId }) {
  const { onlineUser } = useSelector((state) => state.userState);
    ...
    <>
      <div
        ...
        {image ? (
          ...
          <div
            className={` overflow-hidden w-full h-full flex items-center justify-center rounded-full uppercase  text-xl  ${bgColor[randomNumber]}`}>
            {avatarname}
          </div>
        ) : (
